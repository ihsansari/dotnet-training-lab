...
Pinning to a full commit SHA makes the action reference immutable and drastically reduces CI supply-chain risk.

## Why we enforce this with a dedicated workflow

Even if we pin actions today, a future PR could reintroduce `@vX` by mistake.
This workflow fails the PR if any workflow uses non-SHA-pinned actions, keeping the rule permanent and reviewable in CI.
...
That’s exactly the class of failure SHA pinning is designed to prevent.
